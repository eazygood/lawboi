# Dev Script Python Rewrite Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `scripts/dev_local.sh` and `scripts/smoke_local.sh` with a single `scripts/dev.py` that has identical output format and exit-code contract.

**Architecture:** Pure Python stdlib, single file, argparse flag interface (`--down`, `--full`, `--no-ingest`, `--smoke`). All subprocess calls go through a thin `run()` helper. Output format (`PASS / FAIL / WARN / ==>`) preserved exactly so the `test-local` Claude skill parses it unchanged.

**Tech Stack:** Python 3.12 stdlib only — `subprocess`, `urllib.request`, `argparse`, `pathlib`, `signal`, `glob`. No new dependencies.
...
|---|---|---|
| Create | `scripts/dev.py` | Unified dev env management |
| Delete | `scripts/dev_local.sh` | Replaced by `dev.py` |
| Delete | `scripts/smoke_local.sh` | Replaced by `dev.py --smoke` |
| Modify | `.claude/skills/test-local/SKILL.md` | Update script references |

---

### Task 1: Create `scripts/dev.py`

This is the full implementation in one task — the file is too interconnected for incremental TDD; integration testing covers it in Task 2.
...
Create `scripts/dev.py` with this exact content:
...
Expected output contains: `--full`, `--no-ingest`, `--smoke`, `--down` in the options list. Exit code 0.

- [ ] **Step 4: Commit**

```bash
git add scripts/dev.py
git commit -m "feat: add unified scripts/dev.py replacing dev_local.sh and smoke_local.sh"
```

---

### Task 2: Integration verify — `--down`, `--smoke`, and `up`

**Files:** (no changes — verification only)

- [ ] **Step 1: Test `--down` against a stopped environment**

```bash
python scripts/dev.py --down
```

Expected: ` PASS  Environment stopped.` and exit 0 (even with nothing running — pidfiles may not exist).

- [ ] **Step 2: Test `up` brings the stack live**

```bash
python scripts/dev.py
```

Expected sequence of lines ending with:
```
 PASS  Python env ready
 PASS  db healthy
 PASS  migrations applied
==>  db already has N provisions ...   (or ingest output)
 PASS  API healthy at http://localhost:8000
 PASS  UI up at http://localhost:3000
...
- [ ] **Step 3: Test `--smoke` against the running stack**

```bash
python scripts/dev.py --smoke
```

Expected: four PASS lines (`/health`, `/models`, `/search`, `/answer`) and `All hard checks passed.`. Exit code 0.

If `/search` FAILs with 500 — check `/tmp/lawboi_api.log` for the traceback and fix before proceeding.

- [ ] **Step 4: Test `--down` tears everything down**

```bash
python scripts/dev.py --down
```

Expected: `stopped UI`, `stopped API`, ` PASS  Environment stopped.` Exit code 0.

- [ ] **Step 5: Verify `--smoke` exits 1 when API is down**

```bash
python scripts/dev.py --smoke; echo "exit: $?"
```

Expected: ` FAIL  /health -> ...` and `exit: 1`.

---

### Task 3: Update `test-local` skill

**Files:**
- Modify: `.claude/skills/test-local/SKILL.md`

- [ ] **Step 1: Update every script reference in the skill**

Open `.claude/skills/test-local/SKILL.md` and make these replacements:

| Old | New |
|---|---|
| `./scripts/dev_local.sh` | `python scripts/dev.py` |
| `./scripts/smoke_local.sh` | `python scripts/dev.py --smoke` |
| `./scripts/dev_local.sh --full` | `python scripts/dev.py --full` |
| `./scripts/dev_local.sh --no-ingest` | `python scripts/dev.py --no-ingest` |
| `./scripts/dev_local.sh --down` | `python scripts/dev.py --down` |

Also update the **Notes** section: replace any reference to `dev_local.sh` or `smoke_local.sh` with `dev.py`.

- [ ] **Step 2: Verify the skill has no remaining bash script references**

```bash
grep -n "dev_local\|smoke_local" .claude/skills/test-local/SKILL.md
```
...
```bash
git add .claude/skills/test-local/SKILL.md
git commit -m "chore: update test-local skill to use scripts/dev.py"
```

---

### Task 4: Remove old bash scripts

**Files:**
- Delete: `scripts/dev_local.sh`
- Delete: `scripts/smoke_local.sh`

- [ ] **Step 1: Verify no other files reference the old scripts**

```bash
grep -r "dev_local\.sh\|smoke_local\.sh" . \
  --include="*.md" --include="*.sh" --include="*.py" --include="*.yaml" \
  --exclude-dir=".git"
```

Expected: no output. If any file still references them, update it first.

- [ ] **Step 2: Delete the scripts**

```bash
rm scripts/dev_local.sh scripts/smoke_local.sh
```

- [ ] **Step 3: Commit**

```bash
git add -u scripts/dev_local.sh scripts/smoke_local.sh
git commit -m "chore: remove dev_local.sh and smoke_local.sh replaced by scripts/dev.py"
```
...
**Spec coverage check:**
- ✅ One unified script — `scripts/dev.py`
- ✅ Flag interface: `--full`, `--no-ingest`, `--smoke`, `--down`
- ✅ `--smoke` separate, does not start/stop db or API
- ✅ `--full` / `--no-ingest` mutually exclusive (`add_mutually_exclusive_group`)
- ✅ PASS/FAIL/WARN output format preserved exactly
- ✅ ANSI codes only when `sys.stdout.isatty()`
- ✅ Exit 0 on success, exit 1 on FAIL
- ✅ Idempotent `up` — reuses running API/UI, skips ingest if provisions exist
- ✅ LLM key: shell env wins over `.env`
- ✅ Pidfiles at same `/tmp/lawboi_api.pid`, `/tmp/lawboi_ui.pid` paths
- ✅ Logs at same `/tmp/lawboi_api.log`, `/tmp/lawboi_ui.log` paths
- ✅ Runnable with system `python3` before venv exists
- ✅ Old scripts deleted
- ✅ Skill updated

**Placeholder scan:** No TBDs, no "similar to task N", all code blocks complete.

**Type consistency:** `dc` passed explicitly to all docker helpers; `run_q` used consistently for non-raising calls; `run` for check=True calls.
